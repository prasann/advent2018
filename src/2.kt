fun findDuplicate(initVal: Int, frequencies: ArrayList<Int>, dupArray: ArrayList<Int>): Map<String, Int> {
    var sum = initVal
    for (freq in frequencies) {
        sum += freq
        if (!dupArray.contains(sum)) {
            dupArray.add(sum)
        } else {
            return mapOf("Status" to 1, "Val" to sum)
        }
    }
    return mapOf("Status" to -1, "Val" to sum)
}
